...
import { initialPacientes, initialMedicos, initialDiagnosticos, initialMedicamentos, initialExamenes, initialFarmacias, initialFarmaceutas } from "./src/data/initialData.js";
...
async function startServer() {
  ...
  // Ensure DB Schema is in sync and Dr. Nelson is inserted
  try {
  ...
  app.post("/api/auth/request-token", async (req, res) => {
    try {
      ...
      const medico = await db.query.medicos.findFirst({
        where: or(
      ...
      const farmaceuta = await db.query.farmaceutas.findFirst({
        where: or(
      ...
      // 2. Send Real Email if user has email (FREE)
  ...
  app.post("/api/seed", async (req, res) => {
    try {
      ...
      if (pCount.length === 0) {
        // Insert some initial data from the mock if DB is empty
  ...
  app.post("/api/visitas", async (req, res) => {
    try {
      const { visita } = req.body; // Comes from client
      // 1. Insert Visita
  ...
  app.put("/api/recetas/:id/dispensar", async (req, res) => {
    try {
      ...
      if(rResult.length > 0) {
        // Evaluate if the whole visita is completed
  ...
  app.put("/api/visitas/:id/dispensar-todo", async (req, res) => {
    try {
      ...
      // 1. Update visita state to 3 (Burned/Dispensada)
      await db.update(visitas)
        .set({ estado_id: 3 })
        .where(eq(visitas.id, vId));

      // 2. Update all attached recetas to 3 (Burned)
